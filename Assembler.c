...
int parseCodeLine(char *line, int address)
{
    ...
    while (token && numArgs < 4)
    {
        ...
        if (token[0] == ':')
        {
            ...
            snprintf(current.args[numArgs], sizeof(current.args[numArgs]), "%d", labelAddress);
        }
        else
        {
...
void writeBinary(const char *filename)
{
    ...
    header.data_seg_begin = DATA_START_ADDR;
    ...
    // write data segment immediately after code
    for (int i = 0; i < numInstructions; i++)
    {
        if (instructions[i].op[0] == '.' || instructions[i].isCode != 0)
            continue;
...
void validateFile(const char *filename)
{
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        if (line[0] == '\t')
        {
                ...
                // pre-tokenisation: check parentheses balance and ordering
                {
                    ...
                    for (char *p = content; *p; p++)
                    {
                        if (*p == '(')
                        {
                            ...
                            if (lastWasOpen)
                            {
                                fprintf(stderr, "error line %d: consecutive '((' in instruction\n", lineNum);
                                hasError = 1;
                                fclose(f);
                                return;
                            }
                        ...
                        else if (*p == ')')
                        {
                            ...
                            {
                                fprintf(stderr, "error line %d: ')' before matching '('\n", lineNum);
                                hasError = 1;
                                fclose(f);
                                return;
                            }
                    ...
                    {
                        fprintf(stderr,
                                "error line %d: mismatched parentheses (open=%d, close=%d)\n",
                         ...
                         strcmp(opcode, "shftli") == 0 || strcmp(opcode, "shftri") == 0)
                {
                    ...
                    else
                    {
                        ...
                        if (tokens[2][0] != ':')
                        {
                            errno = 0;
                            char *end;
                            unsigned long long val = strtoull(tokens[2], &end, 0);
                            ...
                            {
                                fprintf(stderr,
                                        "error: immediate out of range (must be 0-4095): %s\n",
                                        tokens[2]);
                                hasError = 1;
                            }
                        }
                ...
                else if (strcmp(opcode, "brr") == 0)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else if (errno == ERANGE || val < -2048 || val > 2047)
                            {
                                fprintf(stderr,
            ...
            {
                fprintf(stderr,
                        "error line %d: instruction without .code or .data section\n",
                        lineNum);
                hasError = 1;
                fclose(f);
                return;
